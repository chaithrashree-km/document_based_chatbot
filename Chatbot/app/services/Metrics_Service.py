...
def compute_retrieval_metrics(docs: list, k: int = 3) -> RetrievalMetrics:
    ...
    hit_rate = sum(1 for s in scores if s > 0) / len(scores)
...
class MetricsService:
    ...
    def _log(rec: QueryRecord):
        ...
        if rec.served_from_cache:
            ...
            logger.info(
                ...
                "docs=%d hit=%.2f mrr=%.2f ndcg3=%.2f rerank_avg=%.3f",
                rec.query_id,
                lat.embed_cache_ms or 0,
                lat.search_ms or 0,
                lat.rerank_ms or 0,   lat.llm_ms or 0,
                lat.total_ms or 0,
                ret.docs_retrieved, ret.hit_rate,
                ret.mrr, ret.ndcg_at_3, ret.avg_rerank_score,
            )
    ...
    def _persist(rec: QueryRecord):
        ...
        INSERT INTO query_metrics (
            ...
            docs_retrieved, hit_rate, mrr, ndcg_at_3, avg_rerank_score, avg_context_len
        ) VALUES (
        ...
        values = (
                 ...
                 _f(ret.ndcg_at_3),    _f(ret.avg_rerank_score), _f(ret.avg_context_len),
                )
        try:
            db = Database()
            try:
